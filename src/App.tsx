...
const css = `@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;900&family=Barlow+Condensed:wght@400;600;700&family=Barlow:wght@400;500&display=swap'); *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Barlow', sans-serif; background: ${G.bg}; color: ${G.cream}; } input, textarea, select { font-family: inherit; background: ${G.surface}; color: ${G.cream}; border: 1px solid ${G.border}; border-radius: 6px; padding: 10px 12px; font-size: 14px; } input:focus, textarea:focus, select:focus { outline: none; border-color: ${G.red}; } button { cursor: pointer; font-weight: 600; border-radius: 6px; border: none; transition: all 0.2s; } button:hover:not(:disabled) { opacity: 0.9; } button:disabled { opacity: 0.5; cursor: not-allowed; } table { width: 100%; border-collapse: collapse; } th, td { padding: 12px; text-align: left; border-bottom: 1px solid ${G.border}; } th { background: ${G.card}; font-weight: 700; } a { color: ${G.red}; text-decoration: underline; } a:hover { opacity: 0.8; }`;

const Btn = ({ children, onClick, variant = 'primary', size = 'md', disabled, style = {} }) => {
const base = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: size === 'sm' ? '6px 14px' : '11px 22px', fontSize: size === 'sm' ? 13 : 14, fontWeight: 600, borderRadius: 6, border: 'none', cursor: 'pointer', transition: 'all 0.2s', ...style };
const variants = {
...
<span style={{ display: 'inline-block', padding: '2px 10px', borderRadius: 20, fontSize: 11, fontFamily: '\'Barlow Condensed\', sans-serif', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', background: color + '22', color: color }}>{children}</span>
);
...
💳 <strong style={{ color: G.gold }}>Payment</strong> — All payments are to be settled directly via WhatsApp with me at <strong>+65 9662 5208</strong> before collection. Do not make payment to any bank account or Paynow.
</div>
