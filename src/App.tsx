...
const css = `@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;900&family=Barlow+Condensed:wght@400;600;700&family=Barlow:wght@400;500&display=swap'); *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; } body { background: ${G.bg}; color: ${G.cream}; font-family: 'Barlow', sans-serif; min-height: 100vh; } ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-track { background: ${G.bg}; } ::-webkit-scrollbar-thumb { background: ${G.border}; border-radius: 3px; } input, textarea, select { background: ${G.surface}; border: 1px solid ${G.border}; color: ${G.cream}; border-radius: 6px; padding: 10px 14px; width: 100%; font-family: 'Barlow', sans-serif; font-size: 14px; outline: none; transition: border-color 0.2s; } input:focus, textarea:focus, select:focus { border-color: ${G.red}; } input::placeholder, textarea::placeholder { color: ${G.muted}; } button { cursor: pointer; font-family: 'Barlow Condensed', sans-serif; font-weight: 700; letter-spacing: .06em; border: none; border-radius: 6px; transition: opacity 0.15s, transform 0.1s; } button:active { transform: scale(0.97); } table { border-collapse: collapse; width: 100%; } th, td { padding: 10px 14px; text-align: left; border-bottom: 1px solid ${G.border}; font-size: 13px; } th { font-family: 'Barlow Condensed', sans-serif; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; color: ${G.muted}; font-size: 11px; } tr:hover td { background: #1e1c1a; } .notice-bar { background: linear-gradient(90deg, #1a1209, #211a08, #1a1209); border: 1px solid ${G.gold}33; border-radius: 8px; padding: 16px 20px; margin-bottom: 28px; }`;

const Btn = ({ children, onClick, variant = “primary”, size = “md”, disabled, style = {} }) => {
const base = { display: “inline-flex”, alignItems: “center”, justifyContent: “center”, gap: 6, padding: size === “sm” ? “6px 14px” : “11px 22px”, fontSize: size === “sm” ? 12 : 14, borderRadius: 6, opacity: disabled ? 0.45 : 1, pointerEvents: disabled ? “none” : “auto” };
const variants = {
...
<span style={{ display: “inline-block”, padding: “2px 10px”, borderRadius: 20, fontSize: 11, fontFamily: “‘Barlow Condensed’, sans-serif”, fontWeight: 700, letterSpacing: “.08em”, background: color + “22”, color, border: `1px solid ${color}44` }}>{children}</span>
);
...
{[“Shop”, “Order”].map(n => (
<button key={n} onClick={() => setPage(n)} style={{ background: page === n ? G.red : “transparent”, color: page === n ? “#fff” : G.muted, padding: “5px 12px”, fontSize: 12, borderRadius: 5, fontFamily: “‘Barlow Condensed’, sans-serif”, fontWeight: 700, letterSpacing: “.06em”, border: “none” }}>{n}</button>
))}
<button onClick={() => setPage(“Admin”)} style={{ background: “transparent”, color: G.bg, padding: “5px 12px”, fontSize: 12, borderRadius: 5, border: “none”, userSelect: “none” }}>·</button>
</div>
</div>
</div>
</nav>
...
```
  <div className="notice-bar">
    ...
    <p style={{ fontSize: 13, lineHeight: 1.7, color: G.cream }}>
      All beef is sourced directly from <strong style={{ color: G.gold }}>Mornington Butchery & Pantry</strong>. I am simply collating orders on behalf of our group — I do not process, prepare, or handle any of the meat myself. No cutting, trimming, or preparation will be done by me. For most orders, <strong>arrangements for collection/pickup will need to be made directly with me via WhatsApp</strong> at <a href={`https://wa.me/${OWNER_WA}`} target="_blank" rel="noreferrer" style={{ color: G.gold, textDecoration: "underline" }}>+65 9662 5208</a>.
    </p>
  </div>
...
💳 <strong style={{ color: G.gold }}>Payment</strong> — All payments are to be settled directly via WhatsApp with me at <strong>+65 9662 5208</strong> before collection. Do not make payment to anyone else.
</div>
...
…orders.map(o => [o.name, o.wa, o.items.map(i => i.isEnquiry ? `${i.name}: ${i.enquiryNote}` : i.sliceWeight ? `${i.name} ${i.pieces}pc×${i.sliceWeight}g` : `${i.name} x${i.qty}`).join(”; “), o.total, o.status, new Date(o.ts).toLocaleString()])];
...
const tabStyle = t => ({ background: tab === t ? G.red : “transparent”, color: tab === t ? “#fff” : G.muted, padding: “8px 16px”, fontSize: 13, borderRadius: 5, fontFamily: “‘Barlow Condensed’, sans-serif”, fontWeight: 700, letterSpacing: “.06em”, border: “none” });
  ...
  {tab === "products" && (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <Btn size="sm" onClick={() => setEditProd({ name: "", description: "", emoji: "🥩", unitPrice: 0, unit: "kg", available: true })}>+ Add Product</Btn>
      </div>
...
if (!loaded) return <div style={{ background: G.bg, minHeight: “100vh”, display: “flex”, alignItems: “center”, justifyContent: “center”, color: G.muted, fontFamily: “‘Barlow Condensed’, sans-serif”, letterSpacing: “.12em” }}>LOADING…</div>;
